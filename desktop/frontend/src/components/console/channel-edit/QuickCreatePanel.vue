    ...
    <div class="rounded-xl border border-border/60 bg-card/50 p-4">
      ...
      <div class="space-y-3">
        ...
        <div class="grid gap-3 md:grid-cols-2">
          ...
          <div class="space-y-3">
            <!-- 渠道名称预览 -->
            <div class="flex items-center gap-2 rounded-lg border border-border bg-background/70 p-3">
              <Tag class="h-4 w-4 shrink-0 text-primary" />
              <div class="min-w-0 flex-1 space-y-0.5">
                <div class="text-xs font-semibold">{{ tf('addChannel.channelName', '渠道名称') }}</div>
                <p class="truncate text-[11px] font-medium text-primary">{{ generatedChannelName }}</p>
              </div>
